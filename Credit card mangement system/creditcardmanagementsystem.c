...
void makePayment(struct CreditCard *cc, float amount) {
    ...
    // Ensure payment does not exceed available credit limit
    if (amount > cc->creditLimit) {
        printf("Payment exceeds the available credit limit. Transaction declined!\n");
        return;
    }

    // Subtract the payment amount directly from the credit limit
    cc->creditLimit -= amount;  // Deduct the amount from credit limit
    cc->currentBalance = cc->creditLimit;  // Update current balance to reflect new credit limit

    // Notify the user of the successful payment and updated balance
    printf("Payment of %.2f successful! New remaining credit limit: %.2f\n", amount, cc->creditLimit);

    // Send a message to the user with updated balance details
    char message[100];
    sprintf(message, "Payment successful! Your remaining credit limit is %.2f.", cc->creditLimit);
    sendSMS(cc, message);  // Send SMS with the updated balance

    // Send an email with the updated balance sheet
    sendEmail(cc, "Your updated balance sheet after payment.");
}
